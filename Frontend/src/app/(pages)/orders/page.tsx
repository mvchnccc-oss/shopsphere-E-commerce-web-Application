...
export default async function OrdersPage() {
    ...
    const orders = res as unknown as Orders;
    if (!orders.length) {
        return (
    ...
    const totalSpent = orders.reduce((s, o) =>
        s + o.items.reduce((ss, i) => ss + i.quantity * i.pricePerUnit, 0), 0
    );
    const totalItems = orders.reduce((s, o) =>
        s + o.items.reduce((ss, i) => ss + i.quantity, 0), 0
    );
                                    ...
                                    <tbody>
                                        {order.items.map((item, i) => (
                                            <tr key={i} className="border-t">
                                                <td className="p-3">Product #{item.productId}</td>
